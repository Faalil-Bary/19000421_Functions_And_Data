...
object  ListOfAcc{
	def main(args:Array[String])
	{
		...
		ODlist.foreach(x=>println(x.accNumber+":"+x.balance));
	...
	val interest=(list:List[Account])=>list.map(x=>(new Account(x.nic,x.accNumber,if(x.balance>0) x.balance*1.05d else x.balance*1.01d)));
